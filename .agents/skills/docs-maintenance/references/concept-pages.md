...
1. Read existing flow pages under `brain/domains/<domain>/flows/` for cross-linking.
2. Scan `brain/raw/` recursively and collect files where frontmatter `ingested: false` or `ingested` is absent.
...
6. Derive the output path:
   ...
   - write to `brain/domains/<domain>/concepts/<concept-name>.md`
...
8. Every concept page must include:
   ...
   - `Related flows`
   - `## [Source: <origin>]`
9. Update `brain/domains/<domain>/<domain>.md` to link the concept page if not already present
