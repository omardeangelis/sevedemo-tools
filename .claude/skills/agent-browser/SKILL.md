...
For sites requiring Google/Discord/etc login (replace `https://your-app.example.com` with the target):
...
agent-browser --cdp 9222 open "https://your-app.example.com"
...
agent-browser --headed --session app open "https://your-app.example.com"
# Complete Google OAuth manually in the browser window
# Close when done

# Future runs: cookies persist!
agent-browser --session app open "https://your-app.example.com"
# Already logged in automatically
```

**Chrome profile location:** macOS `~/Library/Application Support/Google/Chrome`, Linux `~/.config/google-chrome`, Windows `%LOCALAPPDATA%\Google\Chrome\User Data`.
