...
## Required Advisor Agents (pre-task)

Before starting a task whose `location` matches a trigger, delegate to the matching project advisor first (see `## Project Advisors` and [references/lifecycle.md](references/lifecycle.md) §6):

- data/schema-layer tasks → the project's schema/data advisor agent, if one is defined
- user-facing-flow tasks → the project's UX/design advisor agent, when UX friction surfaces beyond the spec

If this project defines no advisor agents, skip this step.
...
Project-specific advisor agents detected by init-brain (or added manually). When a task's `location` matches a trigger, delegate via the `Agent` tool with `subagent_type: "<agent-name>"` **before** starting that task.

<!-- init-brain:advisors:start -->
_No project advisor agents detected. If this project defines agents under `.claude/agents/` or `.agents/agents/`, list each here as: **`<name>`** — trigger location — when to delegate._
